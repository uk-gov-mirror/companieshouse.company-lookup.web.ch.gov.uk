#!/bin/bash
#
# Start script for company-lookup.web.ch.gov.uk
...
if [[ -z "${MESOS_SLAVE_PID}" ]]; then
    ...
    source ~/.chs_env/company-lookup.web.ch.gov.uk/env

    PORT="${COMPANY_LOOKUP_WEB_PORT}"
else
    PORT="$1"
    CONFIG_URL="$2"
    ENVIRONMENT="$3"
...
exec java ${JAVA_MEM_ARGS} -jar -Dserver.port="${PORT}" "${APP_DIR}/company-lookup.web.ch.gov.uk.jar"
